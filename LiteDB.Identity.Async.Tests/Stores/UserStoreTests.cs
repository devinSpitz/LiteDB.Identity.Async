...
using LiteDB.Identity.Async.Tests.Mocks;
using LiteDB.Identity.Models;
using Microsoft.AspNetCore.Identity;
using Xunit;

namespace LiteDB.Identity.Async.Tests.Stores
{
    public class UserStoreAsyncTests
    {
        private readonly IServicesBuilder services;
        public UserStoreAsyncTests() {
        ...
        public void AddToRoleAsync()
        {
            // arrange
            var manager = services.GetUserManager();
            var roleManager = services.GetRoleManager();
            
            LiteDbUser newUser = SetUpUser(manager);
            roleManager.Roles.ToList().Should().BeEmpty();
            ...
            roleManager.Roles.ToList().Should().HaveCount(1);
            var role = roleManager.Roles.First();
            
            // act
            manager.AddToRoleAsync(newUser, role.Name).GetAwaiter().GetResult();
            
            // assert
            manager.IsInRoleAsync(newUser, role.Name).GetAwaiter().GetResult().Should().BeTrue();
